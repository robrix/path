  ...
  eff = ElabC . handleSum (eff . handleCoercible) (\case
    Infer (In out span) k -> case out of
      ...
      R (Core.Var n) -> runElabC $ do
        ...
        sigma <- ask
        case res of
          Just t -> elabImplicits (In (Core.Var n) t) (Resources.singleton n sigma) >>= k
          _      -> throwError (FreeVariable n span)
        where elabImplicits tm res
                | Value (Value.Pi _ Im _ t _) <- ann tm = do
                  n <- freshMeta
                  elabImplicits (In (tm Core.:$ In (Core.Var (Local n)) t) (ann tm `vapp` vfree (Local n))) res
...
elabDeclare name ty = do
  elab <- runReader Zero (runContext (runEnv (runConstraints (runElab (generalize ty >>= check Value.type')))))
...
elabDefine name tm = do
  ...
  elab <- runReader One (runContext (runEnv (runConstraints (runElab (maybe infer check ty tm)))))
