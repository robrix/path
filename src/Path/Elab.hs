...
elab (Term (e ::: t)) Nothing = do
  ...
  check e t''
elab (Term (Core Type)) Nothing = pure (Term (Ann Type VType))
elab (Term (Core (Pi n e t b))) Nothing = do
  t' <- check t VType
  env <- ask
  let t'' = eval (erase t') env
  b' <- local (Map.insert (Local n) t'') (check (subst n (Term (Core (Free (Local n)))) b) VType)
  pure (Term (Ann (Pi n e t' b') VType))
elab (Term (Core (Free n))) Nothing = asks (Map.lookup n) >>= maybe (fail ("free  variable: " <> show n)) (pure . Term . Ann (Free n))
elab (Term (Core (f :@ a))) Nothing = do
  ...
  case ann (out f') of
    VPi _ _ t t' -> do
      a' <- check a t
      env <- ask
      pure (Term (Ann (f' :@ a') (t' (eval (erase a') env))))
...
elab (Term (Core (Lam n e))) (Just (VPi tn ann t t')) = do
  ...
  pure (Term (Ann (Lam n e') (VPi tn ann t t')))
elab tm (Just ty) = do
  v <- infer tm
