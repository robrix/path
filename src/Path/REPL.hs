...
script :: ( Carrier sig m
       ...
       => [FilePath]
       -> m ()
script packageSources = evalState (ModuleGraph mempty :: ModuleGraph Qualified (Core Qualified ::: Core Qualified)) (runError loop >>= either (print @Doc) pure)
  where loop = (prompt "λ: " >>= parseCommand >>= maybe loop runCommand . join)
        ...
        runCommand = \case
          ...
          Show Modules -> do
            graph <- get
            let ms = modules (graph :: ModuleGraph Qualified (Core Qualified ::: Core Qualified))
          ...
          Command.Doc moduleName -> do
            ...
            case m :: Maybe (Module Qualified (Core Qualified ::: Core Qualified)) of
