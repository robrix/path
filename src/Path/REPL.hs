...
script :: ( Carrier sig m
       ...
       => [FilePath]
       -> m ()
script packageSources = evalState (ModuleGraph mempty :: ModuleGraph (Spanned (Core Qualified))) (runError loop >>= either (print @Doc) pure)
  where loop = (prompt "λ: " >>= parseCommand >>= maybe loop runCommand . join)
        ...
        runCommand = \case
          ...
          Show Modules -> do
            graph <- get
            let ms = modules (graph :: ModuleGraph (Spanned (Core Qualified)))
          ...
          Command.Doc moduleName -> do
            ...
            case m :: Maybe (Module (Spanned (Core Qualified))) of
...
runParseModule :: FilePath -> (Module (Spanned (Surface.Surface (Name Meta))) -> NamingC (ErrorC Doc (LiftC IO)) a) -> IO (Either Doc a)
