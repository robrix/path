...
eval :: (Carrier sig m, Functor m, Member (Reader (Env QName)) sig) => Term Core a -> m (Value QName)
eval t = asks (flip go t)
  where go d = \case
          In (Core.Var n) _
            | isLocal n -> fromMaybe (vfree n) (Env.lookup n d)
            | otherwise -> vfree n
          In (Core.Lam n b) _ -> Value.Lam (Local n) (go (Env.insert (Local n) (vfree (Local n)) d) b)
          In (f :@ a) _ -> go d f `vapp` go d a
          In Core.Type _ -> Value.Type
          In (Core.Pi n e ty b) _ -> Value.Pi (Local n) e (go d ty) (go (Env.insert (Local n) (vfree (Local n)) d) b)
