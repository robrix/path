...
instance Pretty ElabError where
  pretty (ElabError spans ctx reason) = case reason of
    ...
    TypeMismatch (expected :===: actual :@ _) -> prettyErr spans (fold (punctuate hardline
    ...
    ResourceMismatch n pi used uses -> prettyErr spans msg (prettyCtx ctx <> map prettys uses)
