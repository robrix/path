...
parseModule :: (Carrier sig m, Member (Error Doc) sig, MonadIO m) => FilePath -> m (Module.Module User (Spanned (Surface Var) ::: Spanned (Surface Var)))
parseModule = flip parseFile <*> whole . module'


module' :: (DeltaParsing m, IndentationParsing m) => FilePath -> m (Module.Module User (Spanned (Surface Var) ::: Spanned (Surface Var)))
...
declaration :: (DeltaParsing m, IndentationParsing m) => m (Spanned (Module.Decl User (Spanned (Surface Var) ::: Spanned (Surface Var))))
