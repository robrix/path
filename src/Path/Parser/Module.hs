...
module' :: (DeltaParsing m, IndentationParsing m, MonadFresh m) => FilePath -> m (Module.Module Name (Term (Surface Name Name) Span))
...
declaration :: (DeltaParsing m, MonadFresh m) => m (Module.Decl Name (Term (Surface Name Name) Span))
