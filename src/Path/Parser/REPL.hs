...
command :: (Carrier sig m, Member Parser sig, Member (Reader Lines) sig, Member (Reader FilePath) sig, TokenParsing m) => m (Maybe Command)
typeof, eval, import', doc :: (Carrier sig m, Member Parser sig, Member (Reader Lines) sig, Member (Reader FilePath) sig, TokenParsing m) => m Command
decl :: (Carrier sig m, Member Parser sig, Member (Reader Lines) sig, Member (Reader FilePath) sig, TokenParsing m) => m Command
